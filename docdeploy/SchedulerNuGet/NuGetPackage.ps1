...
function Publish {
  ...
  # DAK: Get nuget config and read specific urls for package and symbols feed
  [xml]$nugetConfig = Get-Content .\NuGet.Config
  $packageSource = ($nugetConfig.configuration.packageSources.add | Where-Object { $_.key -eq "PackageSource" } | Select-Object -First 1).value
  $symbolsSource = ($nugetConfig.configuration.packageSources.add | Where-Object { $_.key -eq "SymbolsSource" } | Select-Object -First 1).value

  Write-Log "Package Source Url: $packageSource"
  Write-Log "Symbols Source Url: $symbolsSource"
  Write-Log " "

  Get-ChildItem *.nupkg | ForEach-Object {
    $url = $packageSource
    if ($_.Name.EndsWith(".symbols.nupkg") -eq $true) {
      $url = $symbolsSource
    }
...
# Modified by DAK: Always create symbol package
$packageTask = Create-Process .\NuGet.exe ("pack Package.nuspec -Symbol")
